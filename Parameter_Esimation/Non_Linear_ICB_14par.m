function [dxdt, add_out] = Non_Linear_ICB_14par(t,x,c,f_1,theta,u,t_vec)
...
% c(3) = Constant Parameter for T_atm
% c(3) = Also constant Parameter for T_water
%
...
dxdt(2) = theta(3)*f_1(3)*(u(5)-x(2)) - theta(4)*f_1(4)*(x(2)-x(1));
% dxdt(3) = c(1)*(x(3)-x(5)); % Equation for Cargo
dxdt(3) = (theta(5)*f_1(5)*u(3) + ...
    (1-u(3))*theta(6)*f_1(6))* ...
    (x(4)-x(3)) - ...
    theta(7)*f_1(7)*u(1)*x(3) +...
    theta(8)*f_1(8)*(u(1)^2) + ...
    theta(9)*f_1(9)*(u(4)-x(3)); 
dxdt(4) = theta(10)*f_1(10)*(x(1)-x(4)) - ...
    (theta(11)*f_1(11)*u(3) + ...
    (1-u(3))*theta(12)*f_1(12))* ... 
    (x(4)-x(3)) + ...
    theta(13)*f_1(13)*(u(5)-x(4))*u(2) + ...
    theta(14)*f_1(14)*u(3); 
